...
-(id)initWithSelectTitle:(NSString *)title viewOfDelegate:(UIView *)view delegate:(id<CDPDatePickerDelegate>)delegate{
    if (self=[super init]) {
       ...
       // [_datePickerView addSubview:_dateLabel];
        _dateConfirmButton=[[UIButton alloc] initWithFrame:CGRectMake(0,0+100,_view.bounds.size.width/2,_view.bounds.size.height*0.07042)];
        [_dateConfirmButton setTitle:@"取消" forState:UIControlStateNormal];
        _dateConfirmButton.userInteractionEnabled=YES;
        [_dateConfirmButton addTarget:self action:@selector(popDatePicker) forControlEvents:UIControlEventTouchUpInside];
        [_dateConfirmButton setTitleColor:[UIColor colorWithRed:0.654 green:0.058 blue:0.142 alpha:1.000] forState:UIControlStateNormal];
        _dateConfirmButton.backgroundColor=[UIColor clearColor];
        [_datePickerView addSubview:_dateConfirmButton];
        
        _dateConfirmButton=[[UIButton alloc] initWithFrame:CGRectMake(_view.bounds.size.width/2,0+100,_view.bounds.size.width/2,_view.bounds.size.height*0.07042)];
        ...
        [_dateConfirmButton setTitleColor:[UIColor colorWithRed:0.582 green:0.089 blue:0.087 alpha:1.000] forState:UIControlStateNormal];
        _dateConfirmButton.backgroundColor=[UIColor clearColor];
        [_datePickerView addSubview:_dateConfirmButton];
...
//    _datePicker.date =[NSDate date];
     ...
     NSString *string=[formatDate stringFromDate:_datePicker.date];
