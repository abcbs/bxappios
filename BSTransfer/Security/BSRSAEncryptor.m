...
#import "BSRSAEncryptor.h"
#import <Security/Security.h>

@implementation BSRSAEncryptor
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
	...
	data = [BSRSAEncryptor stripPublicKeyHeader:data];
	if(!data){
		return nil;
	}
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
	NSData *data = [BSRSAEncryptor encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];
	NSString *ret = base64_encode_data(data);
	return ret;
}

+ (NSString *)encryptString:(NSString *)str publicSecKeyRef:(SecKeyRef )keyRef{
    NSData *data = [BSRSAEncryptor encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicSecKeyRef:keyRef];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
	...
	SecKeyRef keyRef = [BSRSAEncryptor addPublicKey:pubKey];
	if(!keyRef){
		return nil;
	}
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
	NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
	data = [BSRSAEncryptor decryptData:data publicKey:pubKey];
	NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
	return ret;
}
...
+ (NSString *)decryptString:(NSString *)str publicSecKeyRef:(SecKeyRef) keyRef{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [BSRSAEncryptor decryptData:data publicSecKeyRef:keyRef];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
